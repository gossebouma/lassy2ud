  ...
  attribute ud:pos {
         if ($PT eq 'let') then 
            if ( $node/../node[not(@pt='let')] ) then 'PUNCT'
                 ...
                 then 'PUNCT'
                 else 'SYM'  (: brute force solution to avoid multiple heads with punct having other punct as dep (after udapy FixPunct) :) 
...
{  if ($node[@rel="hd" and (@ud:pos="ADP" or ../@cat="pp") ] )  (: vol vertrouwen :)
    then if ($node/../node[@rel="predc"] ) (: met als titel :)
          ...
          then if ($node/../node[@rel=("vc","predc") and (@pos or @cat)])
    	        then local:internal_head_position_with_gapping(($node/../node[@rel="vc"])[1])
    ...
    then if   ($node[../node[@rel=("obj1","se")] and ../node[@rel="hd" and (@pt or @cat)]] )
          then if ( $node/../node[@rel="hd" and @ud:pos="ADP"] ) 
                then local:external_head_position($node/..) (: met als presentator Bruno W :)
                else local:internal_head_position($node/../node[@rel="hd"])
          else if  ( $node/..[@cat=("np","ap") and node[@rel="hd" and (@pt or @cat) and not(@ud:pos=("COP","AUX") ) ]  ]  )  
...
(: we need to select the original node and not the result of 
   ...
   then local:internal_head_position_with_gapping($node/../node[@rel="cnj" and 
   ...
   then if ($node/../node[@rel="hd" and (@pt or @cat)]) (: gapping :)
         then local:internal_head_position($node/..) (: ud head could still be a predc :)
  ...
  (:       else if ($node[@spectype="deeleigen"] )       then "punct" :) (: hack for tagging errors in lassy small 250 :)   
         ...
         then if ($node[@ud:pos="ADP"])
              then if ($node/../node[@rel="predc"]) 
                 ...
                 then "case" (: vol vertrouwen i, Ad U3... , op het gebied van :)
...
{ if      ($gap[@rel="su"])    then "nsubj"
  ...
  else if ($gap[@rel="obj2"])  then if ($head[@ud:pos="ADV"]) then "advmod" else "iobj"
...
(: weather verbs and other expletive subject verbs :) 
              else if ($subj/../node[@rel="hd" and @lemma=("dooien", "gieten", "hagelen", "miezeren", "misten", "motregenen", "onweren", "plenzen", "regenen", "sneeuwen", "stormen", "stortregenen", "ijzelen", "vriezen", "weerlichten", "winteren",  "zomeren" 
) ]  )
                   then "expl"
	           else if ($subj/../node[@rel="hd" and @lemma="ontbreken" and ../node[@rel="pc" and node[@rel="hd" and @lemma="aan"] ] ] ) 
                        then "expl" 
                        else "nsubj"
